...
public interface JobDetailsRepository extends JpaRepository<JobDetails, Integer> {
    ...
    // find batch_id & sub_batch_id
    default List<Object> findBatchIdSubBatchId(EntityManager entityManager, String nativeQuery, int JOB_NUM) {
    ...
    // update job details
    @Transactional
    default Integer updateJobDetailsByBatchIdSubBatchId(EntityManager entityManager, String nativeQuery, int JOB_NUM, int BATCH_ID, int SUB_BATCH_ID, Timestamp CORE_DB_BATCH_END_DATE, int STATUS_NUM, Timestamp CURRENT_BATCH_END_DATE, String Failure_Point, Integer Technical_Error_Code, String Status_Desc, String Technical_Error_Desc) {
        Query query = entityManager.createNativeQuery(nativeQuery);
        query.setParameter(SSIAppConstants.JOB_NUM, JOB_NUM);
